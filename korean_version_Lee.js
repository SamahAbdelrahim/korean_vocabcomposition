...
// Define words array
const words = [{"theword":"책"},{"theword":"눈"},{"theword":"발"},{"theword":"개"},{"theword":"신/신발"},
...
const chosenArray = arrayOfArrays[Math.floor(Math.random() * arrayOfArrays.length)];
shuffleArray(chosenArray);
const selectedWords = chosenArray.slice(0, 19);
const selectedWords3 = chosenArray.slice(20, 40);
...
var opening = {
    ...
    pages: [
        `<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; 
                     ...
                     line-height: 1.6; font-size: 20px; color: #333;">
            <div style="background: #f9f9f9; padding: 30px; border-radius: 10px; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);">
                <p style="font-size: 22px;"><strong>각 단계마다 하나의 단어가 제시됩니다.</strong></p>
                <p style="font-size: 22px;">귀하는 이 단어에 대한 판단을 하게 됩니다.</p>
            </div>
        </div>` 
    ],
...
//timeline.push(practice_solidity1, practice_solidity2)
    ...
    timeline: [
        {
            ...
            questions: [
                {
                    prompt: function() {
                        return `<div style="max-width: 700px; text-align: center; margin: 30px auto; font-size: 22px;">${jsPsych.timelineVariable('theword')}</div>`;
                    },
...
// Debugging: Ensure `selectedWords3` is defined correctly
console.log("selectedWords3:", selectedWords3);
console.log("First item in selectedWords3:", selectedWords3?.[0]);
 ...
 var goodbye = {
    ...
    pages: [
        `<div style="text-align: center; padding: 50px;">
            <img src="stanford.png" style="width: 150px; height: auto; margin-bottom: 20px;">
            <div style="max-width: 600px; margin: 0 auto; padding: 30px; font-size: 28px; background-color: #f8f8f8; 
                        border-radius: 15px; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2);">
                <p style="font-weight: bold; color: #2c3e50;">연구와 과학의 발전에 참여해주셔서 감사드립니다.</p>
                <p style="color: #34495e;">재접속되기 위해서 "다음"을 눌러주세요 ...</p>
            </div>
        </div>`
    ],
    show_clickable_nav: true,
